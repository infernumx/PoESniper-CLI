...
def GetFragmentData(league):
    r = requests.get(
        'http://poe.ninja/api/Data/GetFragmentOverview?league={}'.format(
            league
        )
    )
...
def GetCurrencyData(league):
    r = requests.get(
        'https://poe.ninja/api/Data/GetCurrencyOverview?league={}'.format(
            league
        )
    )
    if r and r.status_code == 200:
...
def GetMapData(league):
    r = requests.get(
        'https://poe.ninja/api/Data/GetMapOverview?league={}'.format(
            league
        )
    )
    if r and r.status_code == 200:
